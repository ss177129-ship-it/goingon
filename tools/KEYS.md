...
> **콘솔 표시 이름은 `goingon STAGING - NOT THE APP`이다(2026-09-11에 바꿈).** 전에는
> "goingon staging"이라 운영 "goingon"과 한 글자 차이였고, APNs 키를 연습실에
> 올려 놓고 운영에 올린 줄 아는 일이 실제로 있었다. 브라우저는 마지막에 본
> 프로젝트를 기억하므로, 콘솔에서 무엇을 만지든 **왼쪽 위 이름부터 본다.**
> 한글은 프로젝트 이름에 허용되지 않는다(Management API가 400을 돌려준다).
