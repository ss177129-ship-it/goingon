...
- [x] **시뮬레이터도 푸시를 받는다.** 이 맥은 T2 칩이 있는 인텔 맥이라 시뮬레이터에 실제 APNs가
      온다 — 23:28에 배너 확인. "인텔이라 시뮬레이터는 불가"는 오진이었고, 못 받던 진짜 원인은
      위 1번 플러그인 버그였다. 앞으로 푸시 발송·수신·탭 라우팅 검증은 시뮬레이터에서 한다
      (계정: 시뮬레이터 = 가파링 `4NuT2u…`, 아이폰 = 빅맨 `Wo4GwW…`)
