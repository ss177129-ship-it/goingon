...
class PacematesScreen extends StatefulWidget {
  const PacematesScreen({super.key});

  @override
  State<PacematesScreen> createState() => _PacematesScreenState();
}

class _PacematesScreenState extends State<PacematesScreen> {
  ...
  /// 친구 목록 구독이 몇 번까지 자동 재시도할지. 인덱스 누락처럼 시간이
  ...
  void _listenFriends() {
    ...
    _friendsSub = _friends.friendsStream(_auth.uid).listen((list) {
      ...
      if (_friendsRetries >= _kMaxRetries) return;
      _friendsRetries++;
    ...
    return ListView(
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(children: [
            Text('페이스메이트', style: GoTheme.serif(22)),
            const Spacer(),
            IconButton(
        ...
        // ── 연결 문제 안내 ──
        if (_friendsError) _connectionNotice(),
        // ── 나에게 온 친구 요청 ──
  ...
  /// 친구 목록 구독이 끊겼을 때 — 조용히 실패하지 않고 알림.
  ...
  Widget _connectionNotice() {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(
          child: Text('페이스메이트 목록을 불러오지 못했어요.',
              style: TextStyle(
                  fontSize: 12,
                  height: 1.4,
                  color: GoColors.ink.withValues(alpha: .7))),
        ),
        TextButton(
          onPressed: () {
            setState(() {
