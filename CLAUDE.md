...
- **러닝 중 라이브 동기화 있음** (2026-08-17 결정으로 이전의 "실시간 동기화 없음"을 폐기). 시속·케이던스·거리를 세션 문서의 `live.{uid}` 맵으로 주고받고, **실세션에서 공명이 실제로 작동한다.** 목표는 라이브 "합산"을 화면에 그리는 것이 아니라 **공명 엔진에 실데이터가 흐르게 하는 것**이다 — 합산은 여전히 완료 후에 한다
