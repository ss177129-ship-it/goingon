...
> **2026-09-02 — 대전제를 제외한 모든 제품 판단이 허물어졌다.** 고객 세그먼트·미션·비전·디자인 철학·수익 모델은 **비어 있다.** 이유와 남은 게이트는 아래 '허문 자리'를 볼 것.
>
> 도전신청서(2026-09-01)는 허물기 **이전**의 제품을 설명한다. 이제 기준선이 아니다 — 새 방향이 정해지면 신청서를 다시 쓸지 정한다.
...
> **2026-09-02 — 디자인 철학도 허물어졌다.** 아래의 무드(따뜻한 종이 질감·크림 배경·세리프 이탤릭)와 색 선택은 **판단이지 제약이 아니다.** 새 방향에 맞지 않으면 전부 바꿀 수 있다. 지금 코드가 이렇게 생겼다는 **현황 기록**으로 읽을 것.
>
> 다만 **구조 하나는 남는다**: 화면에 *나 · 상대 · 우리가 함께 만든 것* 세 자리를 구분하는 것. 색이 무엇이 되든 이 셋이 섞이면 "함께 뛴다"가 화면에서 읽히지 않는다. 지금은 그것이 lime/coral/골드다.

`design/prototype_v2.html` 이 **지금 화면들의 출처**다. 기존 화면을 손볼 때는 열어서 간격·라운딩·타이포를 확인할 것.
...
## 허문 자리 (2026-09-02 피보팅)

> **판단으로 세웠던 결정을 전부 내렸다.** 남긴 것은 기술이 강제하는 것, 법이 요구하는 것, 심사가 막는 것뿐이다.
>
> 왜 허물었나 — 세어 보면 핵심 결정이 거의 전부 **부정형**이었다. 순위 금지, 공개 금지, 과금 금지, 광고 금지, 죄책감 금지, 콜드 DM 금지, 자유 텍스트 최소화, 관계 입구 좁히기, 무통보 거절, 수익화 노출 금지. 제품이 **"하지 않는 것"으로 정의돼** 있었고, 그 목록 중 어느 것도 사람이 앱을 열 이유를 만들지 않았다. CEO 진단: *"소극적이고 방어적인 관계형 앱으로 가고 있다."*
>
> **이 자리는 비워 둔다.** 새 방향이 정해질 때까지 채우지 않는다. 여기 없는 규칙을 근거로 "원래 이랬으니까"라고 말하지 말 것 — 원래는 없다.

### 무엇이 내려갔나 (근거를 잃은 것들)

전부 **재검토 대상**이다. 되살릴 수는 있지만 **새 근거를 세운 뒤에만** 되살린다. 원문은 `git show 1b7a9d6:CLAUDE.md`.

- 순위·리더보드 영구 금지 — 근거가 "기록이 부끄러운 1차 세그먼트"였고, 그 세그먼트가 재검토 대상이 됐다
- 러닝 공개 범위 기본 비공개 — 같은 근거
- 연결 과금 금지 · 러닝 중 삽입 광고 영구 금지 · 수익화 UI 출시 전 노출 금지
- 콜드 DM 금지 · 죄책감 푸시 금지 · 알림은 기대감 문법만
- 무통보 거절 · 자유 입력 텍스트 최소화 · 관계 입구 좁히기
- 승패·순위·앞서기/뒤처지기 **언어** 금지 (가까워짐/나란함/멀어짐만 쓰기)
- 실패음 금지 · 상시 배경음 금지 · 내 발소리 기본 무음
- "기록이 아니라 관계" · "동시는 정점이지 전제조건이 아니다"(D-001) · 8분 에피소드 · 고스트런은 시차 동행
- `docs/redesign/`의 D-001~D-006과 그 아래 설계 전부

**특히 D-001을 짚어 둔다.** "동시에 달리는 것은 정점이지 전제조건이 아니다"는 동시성이 어려우니 비동시(고스트런)로 물러난 결정이었다. 그런데 지키기로 한 대전제 — *멀리서도 서로의 발걸음 소리를 느끼며 함께 뛴다* — 는 본질적으로 동시성 이야기다. **제품이 자기 약속에서 도망친 자리**가 거기다.

### 대전제 (유일하게 남은 제품 결정)

**멀리서도 서로의 발걸음 소리를 느끼며 함께 뛰는 러닝앱.**

이것 하나만 고정이다. 고객 세그먼트·미션·비전·디자인 철학·수익 모델은 전부 열려 있다.

### 허물 수 없는 것 — 게이트

취향이 아니라 **못 넘으면 앱이 존재할 수 없는 선**이다. 피보팅과 무관하게 남는다.

**기술**
- **번들 폰트 밖의 글리프는 두부가 된다.** 이 환경에는 시스템 폰트 폴백이 없다(2026-08-16 확인). 이모지를 쓸 수 없는 이유는 취향이 아니라 이것이다. 다른 문자 집합을 도입하면 반드시 눈으로 확인할 것
- **경과 시간은 타임스탬프 차이로.** iOS는 배경에서 Timer를 멈춘다 — 틱 누적은 반드시 틀린다
- **GPS 필터 네 값**(accuracy 30m, 속도 10m/s, 절대 거리 50m, distanceFilter 5m) — 실측으로 검증됐다. 바꾸면 실내 드리프트가 재발한다. **변경은 승인 필요**
- **집계는 트랜잭션 경유**, username 변경은 단일 트랜잭션 — 우회하면 데이터가 조용히 깨진다
- **목록 규칙에서 `resource.data`는 쿼리 조건으로 채워진다.** `.get(필드, 기본값)`을 쓰면 안 좁힌 쿼리가 기본값으로 통과한다(실제로 비공개 런이 새어 나왔다)
- 백그라운드 위치는 Always 권한이 있을 때만 켠다 — When In Use로 켜면 iOS가 앱을 죽인다

**법**
- 위치정보 수집·이용 **별도 동의**, 이용·제공 사실 확인자료 **자동 기록·6개월 보존**, 위치정보 이용약관 별도 운영, 위치기반서비스사업 신고 (미신고 시 3년 이하 징역 또는 3천만원 이하 벌금)
- 개인정보처리방침·이용약관과 그 공개 URL
- 만 14세 미만 처리

**심사**
- **차단·신고 수단은 필수다.** 사람이 사람에게 닿는 기능이 있는 한 App Store 가이드라인 1.2가 요구한다. **이건 방어적 철학이 아니라 통과 조건**이라 허물 수 없다. 다만 *어떻게* 생겼는지는 전부 열려 있다
- `PrivacyInfo.xcprivacy`와 App Store Connect 개인정보 설문이 **일치**할 것
- 데모 모드 — 핵심이 2인 러닝이라 심사관이 혼자 체험할 유일한 수단이다. 형태는 바꿔도 **혼자서 전체를 볼 수 있는 길**은 남겨야 한다

**물리**
- **달리는 사람은 화면을 오래 볼 수 없다.** 이건 규칙이 아니라 사실이다. 러닝 중 경험을 강하게 만들려면 그 통로는 귀·발·손끝이지 화면이 아니다. (다만 "조작 제로"라는 옛 규칙은 내려갔다 — 무엇을 요구할지는 다시 정한다)

### 살아 있는 자산 (판단이 아니라 사실)

방향이 어디로 가든 이미 만들어져 동작하는 것들이다:
케이던스 검출 · 공명 판정 엔진 · 사운드 재생 층 · 관계 그래프(`follows`) · 러닝 기록과 세션 · 라이브 동기화(`live` 맵) · 보안 규칙과 회귀 테스트 21개 · 자동화 테스트 275개 · 고스트/응원/음성 서비스(화면 없음, `redesign-p5-p6.5` 브랜치에 UI 보존)
...
> 아래는 **기술·안전·정직성**에서 나온 것만 남겼다. 취향과 제품 철학에서 나왔던 금지(실패음·순위 언어·수익화 노출 등)는 2026-09-02에 전부 내려갔다 — '허문 자리' 참조.
...
- **달리는 중에 화면을 오래 보게 만들지 말 것** — 규칙이 아니라 물리다. 통로는 귀·발·손끝이다
