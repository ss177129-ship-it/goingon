...
- 보안 규칙 회귀 테스트(`test_rules/`) 53개 · 자동화 테스트 414개
...
- **규칙 테스트에서 문서를 `withSecurityRulesDisabled`로 심으면 그 쓰기는 검증 밖에 있다.** `friendRequests`가 그렇게 심어져 있어서 수락·거절만 검사됐고, **생성 규칙의 허용 키 목록에 `cheer`가 빠진 것을 18일간 아무도 밟지 않았다**(2026-08-24~09-11, 그동안 페이스메이트 요청은 전부 permission-denied). 화면에는 "처리하지 못했어요"만 뜨고, 기존 계정은 이미 연결돼 있어 목록이 멀쩡해 보였다. **앱이 실제로 던지는 페이로드로 create를 한 번은 밟는 테스트를 둘 것** — 심는 것과 검증하는 것은 다른 일이다
- **필드를 추가하면 규칙의 `hasOnly` 목록을 같이 고쳐야 한다.** 허용 목록 방식은 세션 update뿐 아니라 `friendRequests` create에도 있다. 새 필드는 조용히 거부되고, 거부는 화면에서 안내 문구 한 줄로만 보인다
...
5. **보안 규칙을 만졌으면** `cd test_rules && npm test` — 에뮬레이터 회귀 53개. 규칙은 틀려도 조용해서(느슨하면 아무도 모르고, 빡빡하면 목록이 그냥 빈다) 눈으로는 못 잡는다
