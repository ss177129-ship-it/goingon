...
- **로비 화면(`lobby_screen`)** — **폐기 예정.** '오늘의 상대 선택' + 존재 레이어로 대체한다 (`docs/redesign/product_design_v1.1.md` §2-2, §3). 새 기능을 여기에 얹지 말 것
- **GO? 요청 TTL 30분** — 같은 상대에게 살아 있는 요청이 있으면 재사용(중복 세션 방지). **폐기 예정** — '오늘의 상대 선택' + 존재 레이어로 대체한다 (동 §2-2, §3). 라이브 합류는 v1.1로 미뤄지므로 이 요청 모델 자체가 v1.0 동선에서 사라진다
