...
**멀리 있어도, 발이 맞는 순간을 만든다** — 사운드 리듬을 가진 커뮤니케이션형 러닝 앱(iOS). 핵심 철학: **"기록이 아니라 관계"** — Strava식 퍼포먼스 경쟁이 아니라, 떨어져 있는 두 사람이 발을 맞추는 감각을 판다. **동시에 달리는 것은 정점이지 전제조건이 아니다**(2026-08-22 P0, D-001) — 혼자 달려도 관계에 쌓이고, 겹치면 터진다. 창업자(이찬웅)는 비전공자 1인 개발.
...
**새 화면(개편 P5 이후)의 디자인 기준은 피그마 와이어프레임 + `lib/theme.dart`의 GoColors 토큰이다.** `design/prototype_v2.html`은 기존 화면의 기준으로 남는다. **프로토타입과 `docs/redesign/`이 충돌하면 docs/redesign이 우선**한다 — 색 의미 규칙(나=lime, 상대=coral, 공명=골드 전용)은 양쪽 모두에서 유지.
...
- **~~세션 = 8분 에피소드~~ — 2026-09-01 내려감(CEO 결정).** 코드까지 지웠다: `session_controller.dart`·`chart.dart`·채보 에셋·`main_session_probe`·`session_controller_test`. 되살릴 때는 `git show 85a518b^`부터 보면 된다
  - **왜 지웠나**: 버튼이 지키지 못하는 약속을 하고 있었다. P3가 만든 상태머신이 러닝 화면에 붙은 적이 없고 정식 스템도 없어서, '오늘의 8분 시작'을 눌러도 실제로는 그냥 러닝이 돌았다. 형식만 남고 내용이 없는 상태를 출시 간판으로 둘 수 없다는 판단
  - **딸려 내려간 것**: 홈 2종(입문자/경험자) 분기 — 근거가 에피소드였으므로 한 화면으로 합쳤다. `users.level`과 온보딩 레벨 질문은 **남겼다**(답은 계속 쌓인다. 질문을 뺐다 다시 넣으면 그 사이 가입자의 답만 빈다). 대신 카피에서 "홈이 바뀐다"는 약속을 걷어냈다
  - **살아남은 것**: 시간 기반이라는 판단 자체는 유효하다 — 거리 단위는 빠른 사람의 1km와 입문자의 1km가 다른 시간이라 '같은 것을 함께'가 깨진다(§2-1). 에피소드를 다시 세운다면 이 근거 위에 세운다. `docs/redesign`의 D-004·D-005는 **문서가 코드보다 앞서 있는 상태**이니 그대로 읽지 말 것
- **v1.0에서 러닝은 자유런과 고스트런 둘뿐이다.** 입구는 '오늘의 상대'(`today_partner_screen`) 하나
...
- **라이브 동기화는 v1.1 범위** — 구현은 살아 있고 되돌리지 않지만(아래 '기존 결정' 참조) **v1.0에는 넣지 않는다.** v1.0에서 '함께'를 담당하는 것은 **고스트런과 응원**이다. 동시성은 정점이지 전제조건이 아니며, 전제조건으로 두면 앱을 열 이유가 "상대가 자유로운 드문 순간"에 갇힌다 (D-001, D-005, §8 범위표)
...
- **데모 모드** (`demo: true`, lobby/run/finish 관통) — 가상 파트너 '지수'와 전체 흐름 체험. **Apple 심사관용이므로 절대 제거 금지** (핵심 기능이 2인 동시 러닝이라 심사관이 혼자 체험할 유일한 수단). 친구 0명·기기 1대·권한 없음 상태에서도 반드시 진입 가능해야 하고, 진입점은 설정('혼자 미리 체험하기')에 **항상 유지**. 데모 러닝은 RunRecovery에 저장하지 않는다
  - **내용물은 '데모런(온보딩 페이서)'과 통합 예정** — 심사관용 진입점은 그대로 두되, 안에서 도는 것을 온보딩의 데모런과 같은 것으로 만든다(가상 페이서임을 명시). 과제는 TODO.md P5
- **친구 연결은 '요청 → 수락' 모델** — 아이디로 찾아 요청을 보내고 상대가 수락해야 연결된다. 아이디를 아는 사람이 곧 연결 권한이 되는 구조는 폐기함
  ...
  - 개편 후 관계의 입구는 **팔로우(페이스메이트)** 로 바뀐다 — 초대 링크·공명 후 맺기·릴레이 상호 동의. 차단과 무통보 거절 원칙은 그대로 승계 (§6-1)
- **완료 화면 공유 카드** — RepaintBoundary 캡처 → share_plus. 셋로그식 성장 엔진이라 완성도 유지가 전략적으로 중요

### 폐기 완료 (2026-08-23, P5)

- **로비 화면(`lobby_screen`) 삭제됨.** 대체는 `today_partner_screen`('오늘, 누구와 겹칠까요?'). 로비는 서로가 지금 자유로운지 기다리는 방이었고, 그 기다림이 앱을 여는 이유를 "상대가 한가한 드문 순간"에 가둬 놨다
- **GO? 요청 동선 제거됨** — 화면에서 보내고 받는 길이 없다. `RunService.createSession`/`incomingSessions`/`pushLive`와 세션 규칙·`live` 맵은 **지우지 않았다**(라이브 합류 v1.1). 다시 열 때는 문만 다시 달면 된다
- **탭바는 홈 · 여정 · 프로필** 셋이다. 와이어프레임의 네 번째 '서랍'은 화면이 생기는 P7에서 붙인다. 옛 홈의 친구 UI는 `pacemates_screen`으로 옮겨 설정에서 들어간다
- **세션 없는 러닝이 있다** — `RunScreen(sessionId: '')`. 자유런과 고스트런이 그것이고, 세션 관련 경로(제스처 구독·live 쓰기·세션 결과 제출)를 전부 건너뛴다. `RunRecovery` 스냅샷도 빈 sessionId를 만날 수 있다
...
4. 화면 작업이면: `xcrun simctl io booted screenshot /tmp/check.png`로 스크린샷을 찍어 직접 보고 **기준 문서와 비교한 결과**를 보고할 것 — 기존 화면은 prototype_v2.html, 개편 화면은 피그마 와이어프레임 + GoColors 토큰. ("확인했음" 한 줄 금지 — 무엇이 일치하고 무엇이 다른지 명시)
