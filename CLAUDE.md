...
- **세션 = 8분 에피소드** — 시간 기반, 완결 내장, **상한이 아니다**, 자유런과 병존한다. 거리 단위는 빠른 사람의 1km와 입문자의 1km가 다른 시간이라 '같은 것을 함께'가 깨진다. 시간은 평등하고 공유 가능하다. 8분 뒤 30초의 잔향 구간에서 계속 달리면 자유런으로 이음새 없이 이어지고, 걷기로 바뀌면 종료 시퀀스로 간다. 입구는 분리(에피소드런/자유런), 출구는 연속 (§2-1, §2-2)
...
- **러닝 중 화면 조작 제로** — 시작 후 종료까지 화면을 보거나 만져야 하는 순간이 0이어야 한다. 러닝 중 입력은 셋뿐: 이어폰 꼭지 1회 누름(문맥적 예/아니오) · 꼭지 길게 누름(워키토키) · 발(케이던스). 화면에 두는 조작 요소는 일시정지만. **모든 선택(에피소드·상대·모드)은 시작 전에 끝낸다** (§0 원칙 1)
