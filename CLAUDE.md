...
- **이모지 글리프 사용 금지** — 2026-08-13 시뮬레이터에서 직접 검증함: `fontFamilyFallback`에 `Apple Color Emoji`를 넣어도 **본문·세리프 양쪽 모두 ?(두부)로 깨짐**. 한글 폴백과는 다른 원인임(한글은 NotoSansKR 폴백으로 해결됨). 두부 모양이 특정 폰트의 .notdef 글리프라, 폴백 사슬이 이모지 코드포인트에서 시스템 폰트로 넘어가기 전에 멈추는 것으로 보임 — 다시 시도할 사람을 위한 단서. **Material Icons 또는 텍스트만 사용**
  - **2026-08-16 재검증: 배포 타깃을 iOS 26으로 올린 뒤에도 동일하게 깨짐.** iOS 버전 제약이 아니므로 최소 버전 인상으로는 풀리지 않는다. 같은 화면에서 대조군(`가 A 1 ▲`)은 정상 렌더됐으므로 폰트 설정 자체는 멀쩡함
  - **원인: 이 환경에는 시스템 폰트 폴백이 아예 작동하지 않는다.** 2026-08-16에 세 조건을 나란히 렌더해 확인함(시뮬레이터):

    | 조건 | 이모지 | 한글 | `A 1 ▲` |
    |---|---|---|---|
    | `fontFamily` 미지정 (시스템 기본) | 두부 | **두부** | 정상 |
    | NotoSansKR + `Apple Color Emoji` 폴백 | 두부 | 정상 | 정상 |
    | NotoSansKR 단독 (폴백 없음) | 두부 | 정상 | 정상 |

    폰트를 지정하지 않으면 **한글조차 두부**다. 즉 한글이 나오는 건 폴백이 일해서가 아니라 NotoSansKR을 번들해 **이름으로 직접 지정**했기 때문이다. 폴백 유무(2행 vs 3행)가 아무 차이를 못 만드는 것도 같은 얘기다
  - 따라서 **`fontFamilyFallback`에 시스템 폰트 이름을 넣는 방법은 다시 시도하지 말 것.** 두 번 실패했고, 위 표가 이유를 설명한다
  - **함의: 번들 폰트에 없는 글리프는 안전망이 없다.** InstrumentSerif·NotoSansKR이 못 덮는 문자를 쓰면 조용히 두부가 된다. 새 문자 집합(다른 언어, 특수 기호)을 도입할 땐 반드시 시뮬레이터에서 눈으로 확인할 것
  - 굳이 이모지를 써야 한다면 남은 길은 하나 — **이모지 폰트를 `assets/fonts/`에 함께 번들**해 에셋 쪽으로 끌어오는 것. 다만 Noto Color Emoji가 10MB 안팎이라 IPA가 30% 가까이 커진다. 지금은 Material Icons로 충분하므로 **비용이 이득보다 크다는 판단**이며, 프로토타입의 이모지도 대부분 자리표시자라 앱의 대체물(InitialAvatar·실제 사진·Material Icons)이 이미 더 낫다
  - **기하 기호(▲ ● ★ 등)는 이모지가 아니라 일반 글리프라 정상 렌더된다.** NotoSansKR에 포함돼 있어 그대로 쓸 수 있음
  - **예외: Flutter가 그리지 않는 문자열에는 이모지를 써도 된다.** 공유 시트로 넘기는 텍스트(`Share.shareXFiles(text: ...)`), 푸시 알림 본문 등은 받는 쪽(메시지 앱·iOS)이 시스템 폰트로 렌더하므로 깨지지 않는다. `finish_screen.dart`의 공유 문구가 이 경우라 규칙 위반이 아님
